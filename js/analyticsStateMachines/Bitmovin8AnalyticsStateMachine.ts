...
export class Bitmovin8AnalyticsStateMachine implements AnalyticsStateMachine {
  ...
  createStateMachine(opts: AnalyticsStateMachineOptions) {
    this.stateMachine = StateMachine.create({
      ...
      events: [
        {name: Event.SOURCE_LOADED, from: [this.States.SETUP, this.States.ERROR], to: this.States.READY},

        {name: Event.PLAY, from: this.States.READY, to: this.States.STARTUP},
        {name: Event.PLAYING, from: this.States.READY, to: this.States.PLAYING},
        {name: Event.READY, from: this.States.READY, to: this.States.READY},
        {name: Event.VIDEO_CHANGE, from: this.States.READY, to: this.States.READY},
        {name: Event.AUDIO_CHANGE, from: this.States.READY, to: this.States.READY},

        {name: Event.PLAYING, from: this.States.STARTUP, to: this.States.PLAYING},
        ...
        {name: Event.READY, from: this.States.STARTUP, to: this.States.STARTUP},

        {name: Event.PLAYING, from: this.States.PLAYING, to: this.States.PLAYING},
      ...
      callbacks: {
        ...
        onbeforeevent: (event, from, to, timestamp, eventObject) => {
          if (event === Event.SEEK && from === this.States.PAUSE) {
            ...
            if (timestamp - this.pausedTimestamp < Bitmovin8AnalyticsStateMachine.PAUSE_SEEK_DELAY) {
          ...
          if (event === Event.SEEKED && from === this.States.PAUSED_SEEKING) {
            ...
            }, Bitmovin8AnalyticsStateMachine.SEEKED_PAUSE_DELAY);
            return false;
          }
